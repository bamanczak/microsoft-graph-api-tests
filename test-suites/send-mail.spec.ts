...
describe('Send e-mail tests: ', () => {
    ...
    it('Should deliver simple e-mail as written', () => {
        ...
        const testMailSubject = `[POTATO] Test sent on: ${date}`;
        const testMailContent = '<h1>This is a test potato with email</h1>It has a link to <a href="https://en.wikipedia.org/wiki/Potato">the potato wiki page</a>.';
        const testMail: MicrosoftGraph.Message = {
        ...
        return mailApi
            .sendEmail(user1_accessToken, testMail)
    ...
    it('Should deliver e-mail with an attachment, as written', () => {
        const date: Date = new Date();
        const testMailSubject = `[POTATO] Response Test sent on: ${date}`;
        const testMailContent = 'In the attached file you can find a nice potato';
        const attachmentOne = {
        ...
        return mailApi
            .sendEmail(user2_accessToken, testMail)
            .then((response) => expect(response).to.have.status(202))
            .then(() => mailHelper.sleep(3000)) // wait for e-mail to be delivered
